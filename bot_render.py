...
@flask_app.route('/')
def home(): return "Bot VVF LIVE PRO attivo - FIXED"
def run_flask():
...
def get_keyboard():
    return InlineKeyboardMarkup([[InlineKeyboardButton("🏙️ 5km Città", callback_data="raggio_5"), InlineKeyboardButton("🏘️ 10km", callback_data="raggio_10")],[InlineKeyboardButton("🚗 15km", callback_data="raggio_15"), InlineKeyboardButton("🚒 30km", callback_data="raggio_30")],[InlineKeyboardButton("🗺️ 50km Provincia", callback_data="raggio_50")]])
...
async def start_cmd(u,c):
    if u.effective_chat.id!=MY_CHAT_ID: return
    stato="✅ LIVE attiva" if location_set else "⚠️ non impostata"; await u.message.reply_text(f"🚒 *Bot VVF Live PRO FIXED*\n\nPosizione: {stato}\nCentro: {current_lat:.5f}, {current_lon:.5f}\nRaggio: {int(current_radius_m/1000)}km", parse_mode="Markdown", reply_markup=get_keyboard())
...
def main():
    load_location()
    # FIX per Python 3.14 su Render: crea event loop prima di tutto
    ...
    print(f"Avviato LIVE PRO FIXED - {current_radius_m/1000}km")
    app.run_polling()
